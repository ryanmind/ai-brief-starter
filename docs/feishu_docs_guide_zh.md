...
本项目已支持自动把日报写入飞书新文档（不覆盖旧文档），你只需维护一个“总览入口”即可。建议流程：

1. 在飞书建一个文档：`AI早报总览`。
2. 将该文档链接配置为 `FEISHU_REPORT_DOC_URL`（作为固定入口）。
3. 每天定时任务会新建“当天完整文档”并自动写入全文。
4. 飞书群消息会自动附“今日完整文档（飞书）”链接。
5. 如需人工复盘，再把关键结论汇总到总览文档。
...
2. 质量检查
3. 上传运行产物为 `ai-brief-reports` artifact（不回写仓库）
...
7. 需要原始文件时，可在该次运行下载 `ai-brief-reports` artifact。
