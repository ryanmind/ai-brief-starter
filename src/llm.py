...
def llm_chat(
    ...
    """Call LLM chat with caching, observability logging, and failover.
    ...
    Args:
        ...
        max_retries: Maximum retry attempts on failure per model (default: 3)
        task_name: Name of the task for logging (default: "unknown")
        fallback_models: List of fallback models to try if primary model fails (default: from config)

    Returns:
        Trimmed response text from LLM

    Raises:
        Exception: If all retry attempts and all fallback models fail
    """
    from src.config import get_fallback_llm_models
    if fallback_models is None:
        fallback_models = get_fallback_llm_models()

    # Build list of models to try: primary first, then fallbacks
    models_to_try = [model] + fallback_models

    # Check cache first for the primary model
    cache = _get_cache()
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    # All retries exhausted for this model, move to next fallback
                    logger.warning(
