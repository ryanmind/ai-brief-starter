...
def test_quality_check_soft_mode_does_not_fail(tmp_path, monkeypatch):
    ...
    report.write_text(REPORT_WITH_KEY_POINT_ISSUE, encoding="utf-8")
    monkeypatch.setenv("QUALITY_CHECK_STRICT", "0")
    monkeypatch.setenv("KEY_POINTS_MIN_COUNT", "2")
...
def test_quality_check_autofix_repairs_and_passes(tmp_path, monkeypatch):
    ...
    assert "细节：" not in content
    lines = content.splitlines()
    key_header_idx = next(
        (idx for idx, line in enumerate(lines) if ("关键点" in line and line.lstrip().startswith(("-", "*")))),
        -1,
    )
    assert key_header_idx >= 0
    key_point_lines: list[str] = []
    for line in lines[key_header_idx + 1 :]:
        if line.startswith("- 来源"):
            break
        if line.lstrip().startswith("- "):
            key_point_lines.append(line)
    assert len(key_point_lines) >= 2
...
### 1) 测试标题
- 摘要：公司完成1.2亿美元融资，2026年收入增长50%，涉及监管审批流程。
- 关键点：
  - 融资信息披露
  - 监管流程推进
- 影响：行业竞争加速。
